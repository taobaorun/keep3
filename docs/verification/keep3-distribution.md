...
| tap ownership and token | In progress | Public `taobaorun/homebrew-keep3` has protected `main`; its required GitHub Actions `cask validation` check passes on pull requests and post-merge pushes, and a repository-scoped fine-grained token is stored in the protected environment. The single-maintainer two-run approval contract is staged; clean install/upgrade/uninstall on macOS 14+ remains pending |
| gh-pages setup | In progress | Protected `gh-pages` exists as the HTTPS-enforced Pages source and its initial build passed. The first protected publication must still verify canonical endpoints, caching, expiry, immutable paths, and rollback rejection |
