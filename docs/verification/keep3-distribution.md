...
| tap ownership and token | In progress | Public `taobaorun/homebrew-keep3` exists with `main`; a repository-scoped fine-grained token is stored in the protected environment. The single-maintainer two-run approval contract is staged; tap CI, branch protection, and clean install/upgrade/uninstall on macOS 14+ remain pending |
