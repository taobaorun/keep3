...
- KTD3. **Encode the signature transition as resolved visual tokens, not a user preference.** Resolve content at 150 milliseconds, component changes at 210 milliseconds, expansion at 270 milliseconds, and collapse at 200 milliseconds; resting content has no repeat animation. Reduce Motion resolves every spatial/shape transition to a 120-millisecond crossfade, while Reduce Transparency, increased contrast, and differentiate-without-color resolve at render time. Governs visual R2–R7, R11, R14, R16.
...
- **Test scenarios:**
  ...
  2. Covers visual AE2. A secondary focus completes the 150-millisecond content handoff inside the persistent shell and keeps an outlined `2` or `3` marker.
